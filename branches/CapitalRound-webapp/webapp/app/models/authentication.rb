...
class Authentication < ActiveRecord::Base
  ...
  validates_uniqueness_of :username
  validates_format_of :username, :with => /[a-z0-9!\#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!\#$%\&'\*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?:[A-Z]2}|com|org|net|edu|gov|mil|biz|info|mobi|name|aero|asia|jobs|museum|li|ru)/i, :message => "Invalid email"
  ...
  def self.check_errors(authentication_info)
    ...
    return "You entered invalid email address" if !authentication_info[:username] =~ /[a-z0-9!\#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!\#$%\&'\*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?:[A-Z]2}|com|org|net|edu|gov|mil|biz|info|mobi|name|aero|asia|jobs|museum|li|ru)/i
    return "You entered too short.long password" if !authentication_info[:username].length.between?(3, 60)
    return "You entered too short.long password" if !authentication_info[:password].length.between?(3, 20)
    auth = find(:first, :conditions=>["username = ?", authentication_info[:username]])
    return "This username already exists." if auth != nil
    nil
  end
