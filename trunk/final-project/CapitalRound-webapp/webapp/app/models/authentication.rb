...
class Authentication < ActiveRecord::Base
  ...
  def self.check_errors(authentication_info)
    ...
    elsif  !authentication_info[:username] =~ /[a-z0-9!\#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!\#$%\&'\*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?:[A-Z]2}|com|org|net|edu|gov|mil|biz|info|mobi|name|aero|asia|jobs|museum|li|ru)/i
      return "You entered invalid email address"

    else

      if !authentication_info[:username].length.between?(3, 60)
        return "User name should be between 3 - 60 characters"

      elsif !authentication_info[:password].length.between?(3, 20)
        return "Password should be between 3 - 20 characters"
      else
